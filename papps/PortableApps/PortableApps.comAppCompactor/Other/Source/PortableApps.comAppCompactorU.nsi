;Copyright 2006-2022 John T. Haller
...
Function .onInit
	...
	${If} $0 == 0
		StrCpy $intBitDepth "32"
	${Else}
		StrCpy $intBitDepth "64"
	${EndIf}
...
;		DetailPrint "ext=[$2]"
		...
		CompressFile:
			...
			nsExec::ExecToStack `"$EXEDIR\App\bin\upx$intBitDepth.exe" $UPXCMDLINE ".$0\$1"`
			Pop $3  ;    exit code
			Pop $R3	;    UPX output
			IntOp $intCurrentFileProcessing $intCurrentFileProcessing + 1
			StrCmp $3 "error" 0 +6
				MessageBox MB_OK|MB_ICONEXCLAMATION "Unable to execute UPX ($EXEDIR\App\bin\upx$intBitDepth.exe)$\n$\nTry reinstalling ${APPNAME}."
				DetailPrint " "
				DetailPrint "***  Aborted!  ***"
			...
			StrCmp $COMPRESSTYPEDECOMPRESS "1" NextFile
				...
				SetDetailsPrint ListOnly
				nsExec::ExecToStack `"$EXEDIR\App\bin\upx$intBitDepth.exe" -t ".$0\$1"`
				Pop $3	;   exit code
				Pop $R3	;   UPX output
		...
		RecompressZIP:
			...
			SetDetailsPrint ListOnly
			nsExec::Exec `"$EXEDIR\App\bin\7z$intBitDepth\7z.exe" x ".$0\$1" -o"${ZIPTEMP}"`
			Pop $3	;   exit code
			StrCmp $3 "error" 0 +6
				MessageBox MB_OK|MB_ICONEXCLAMATION "Unable to execute 7-Zip ($EXEDIR\App\bin\7z$intBitDepth\7z.exe)$\n$\nTry reinstalling ${APPNAME}."
				DetailPrint " "
				DetailPrint "***  Aborted!  ***"
				RealProgress::SetProgress /NOUNLOAD 100
				Abort
			;DetailPrint "          Compressing..."
			nsExec::Exec `"$EXEDIR\App\bin\7z$intBitDepth\7z.exe" a -tzip ".$0\$1~" "${ZIPTEMP}\*" -r -mx=7`
			Pop $3	;   exit code
			IntOp $intCurrentFileProcessing $intCurrentFileProcessing + 1
			StrCmp $3 "error" RecompressError
			StrCmp $3 "0" 0 RecompressError
				nsExec::Exec `"$EXEDIR\App\bin\7z$intBitDepth\7z.exe" t  ".$0\$1~"`
				Pop $3
				StrCmp $3 "error" RecompressError
				StrCmp $3 "0" 0 RecompressError
					SetDetailsPrint none
